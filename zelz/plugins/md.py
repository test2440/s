    ...
    try:
        ...
        if message.media:
            ...
            if message.text:
                ...
                await zedub.send_message(event.chat_id, f"ᯓ 𝗦𝗼𝘂𝗿𝗰𝗲 𝙈𝙖𝙏𝙍𝙞𝙭 ⌁ - حـفـظ المـحتـوى 🧧\n⋆┄─┄─┄─┄┄─┄─┄─┄─┄┄⋆\n**⌔╎ تـم جلب المنشـور المقيـد .. بنجـاح ☑️** ❝\n**⌔╎رابـط المنشـور** {post_link} .", reply_to=ahmed)
                os.remove(file_path)
                await event.delete()
            else:
                await zedub.send_file(event.chat_id, file=file_path, caption=f"[ᯓ 𝗦𝗼𝘂𝗿𝗰𝗲 𝙈𝙖𝙏𝙍𝙞𝙭 ⌁ - حـفـظ المـحتـوى 🧧](t.me/VEEVVW) .\n⋆┄─┄─┄─┄┄─┄─┄─┄─┄┄⋆\n**⌔╎ تـم جلب المنشـور المقيـد .. بنجـاح ☑️** ❝\n**⌔╎رابـط المنشـور** {post_link} .")
                os.remove(file_path)
                await event.delete()
        else:
            if message.text:
...
async def zelzal_ss(event):
    ...
    async with borg.conversation(chat) as conv:
        ...
        await borg.send_file(
            event.chat_id,
            zedthon1,
            caption=f"<b>⎉╎تم تحميـل الستـوري .. بنجـاح 🎆\n⎉╎الرابـط 🖇:  {zilzal}\n⎉╎تم التحميـل بواسطـة <a href = https://t.me/VEEVVW/1>𝙈𝙖𝙏𝙍𝙞𝙭 ⌁</a> </b>",
            parse_mode="html",
        )
        await zzz.delete()
        await delete_conv(event, chat, purgeflag)
...
async def _(event):
    ...
    await zedub.send_file(
        event.chat_id,
        msg4[0],
        caption=f"<b>⎉╎تم جلب المـوقـع .. بنجـاح \n⎉╎</b><code>{con}</code>  🗺 \n<b>⎉╎بواسطـة <a href = https://t.me/VEEVVW/1>𝙈𝙖𝙏𝙍𝙞𝙭 ⌁</a> </b>",
        parse_mode="html",
    )
...
async def zelzal_ai(event):
    ...
    async with borg.conversation(chat) as conv:
        ...
        await borg.send_message(
            event.chat_id,
            f"<b>{ztxt}\n\n<a href = https://t.me/VEEVVW/1>𓆩 𝙈𝙖𝙏𝙍𝙞𝙭 ⌁ 𝗨𝘀𝗲𝗿𝗯𝗼𝘁 𓆪</a> </b>",
            parse_mode="html",
            reply_to=reply_message,
        )
        await zzz.delete()
    ...
    async with borg.conversation(chat) as conv: 
        ...
        try:
            ...
            if "Find books with" in malath1: 
                zz1 = malath1.replace("Find books with @GoogleBooksSearchBot", "📕\n<b>- تم التحميـل بواسطـة <a href = https://t.me/VEEVVW/1>𝙈𝙖𝙏𝙍𝙞𝙭 ⌁</a> </b>") 
                await zed.delete()
                await borg.send_file(
                    event.chat_id,
                    zedthon1,
...
async def zelzal_gpt(event):
    ...
    async with borg.conversation(chat) as conv: 
        ...
        ztxt = zlz.message
        await zed.delete()
        await borg.send_message(event.chat_id, f"[ᯓ 𝙈𝙖𝙏𝙍𝙞𝙭 ⌁ 𝗖𝗵𝗮𝘁𝗚𝗽𝘁 -💡-](t.me/VEEVVW) **الذكاء الاصطناعي\n⋆──┄─┄─┄───┄─┄─┄──⋆**\n**• س/ {question}\n\n• {ztxt}**", link_preview=False)
        await delete_conv(event, chat, purgeflag)
...
async def zelzal_gpt(event):
    ...
    await zed.edit(f"[ᯓ 𝙈𝙖𝙏𝙍𝙞𝙭 ⌁ 𝗖𝗵𝗮𝘁𝗚𝗽𝘁 -💡-](t.me/VEEVVW) **الذكاء الاصطناعي\n⋆──┄─┄─┄─┄─┄─┄──⋆**\n**• س/ {question}\n\n• {answer}**", link_preview=False)
